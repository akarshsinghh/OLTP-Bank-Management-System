...
INSERT INTO Account VALUES (12494980148173100, 'Checking', 'DMDD0333100', 100007, 300);
INSERT INTO Account VALUES (8744336085399580, 'Checking', 'DMDD0101022', 100008, 1100);
...
INSERT INTO LOAN values(61292734,100007,'DMDD0454500',20000,10000, 2,'Active',115005,TO_DATE('2019-05-10','YYYY-MM-DD'),10.00);
INSERT INTO LOAN values(61292799,100008,'DMDD0101022',24000,18000, 2,'Active',115005,TO_DATE('2019-05-10','YYYY-MM-DD'),10.00);
...
-- Number of Employees who work under a specific supervisor 

select employee_id,first_name ||' '|| last_name as name,department,
emp_under,salary from (select * from (SELECT supervisor_id,COUNT(*) as EMP_UNDER FROM Employee GROUP BY supervisor_id) emp
inner join employee e on e.employee_id=emp.supervisor_id)
order by salary desc, emp_under desc;

-- Top 3 Employees from every branch who have the highest account balance

select * from (select t.*,
dense_rank () over (partition by branch_id order by account_balance desc) as rank from (select c.customer_id, c.first_name ||' '|| last_name as name, c.street, c.city
,c.account_balance, b.branch_id from (select c.*,a.account_balance,a.branchifsc from customer c inner join account a
on c.customer_id = a.customer_id) c left join branch b on b.branchifsc = c.branchifsc) t) where rank <= 3;

-- Potential Defaulters 

SELECT CUSTOMER.FIRST_NAME, CUSTOMER.LAST_NAME, CUSTOMER.CUSTOMER_ID, ACCOUNT.ACCOUNT_BALANCE, LOAN.OUTSTANDING AS DEBT
FROM CUSTOMER
INNER JOIN ACCOUNT ON CUSTOMER.CUSTOMER_ID= ACCOUNT.CUSTOMER_ID
INNER JOIN LOAN ON CUSTOMER.CUSTOMER_ID= LOAN.CUSTOMER_ID
WHERE ACCOUNT.ACCOUNT_BALANCE < LOAN.OUTSTANDING/10;

-- total money lent by branch, loan count, accounts at branch summary

select branch_id,city,zipcode,loan_count,
total_debt,branch_accounts from (select * from (select b.branchifsc,b.branch_id, b.city,b.zipcode,l.loan_count from 
branch b right join (select LOANIFSC,count(*) as loan_count from loan group by LOANIFSC) l
on b.branchifsc=l.loanifsc) lc
left join(select loanifsc,sum(outstanding)as total_debt from loan group by loanifsc) td
on lc.branchifsc=td.loanifsc
left join
(select branchifsc, count(*) branch_accounts from account group by branchifsc) ba
on lc.branchifsc = ba.branchifsc);