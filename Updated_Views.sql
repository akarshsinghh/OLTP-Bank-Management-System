...
CREATE OR REPLACE PROCEDURE EMPLOYEE_LOGIN(EMP_ID IN NUMBER, BRANCH IN NUMBER, DEPT IN VARCHAR, FIRST_NAME IN VARCHAR, LAST_NAME IN VARCHAR, SEX IN VARCHAR, DOB IN DATE, SAL IN NUMBER, EROLE IN VARCHAR, EID IN VARCHAR, CNO IN NUMBER, SUP IN NUMBER)
AS
COUNT_E NUMBER := 0;
EXIST EXCEPTION;
INVALID EXCEPTION;
...
IF COUNT_E = 0 THEN
    ...
    IF (EMP_ID > 0) AND (BRANCH > 0) AND LENGTH(DEPT) > 0 AND LENGTH(FIRST_NAME) > 0 AND LENGTH(LAST_NAME) > 0 AND SAL > 0 AND LENGTH(EROLE) > 0 AND LENGTH(EID) > 0 AND CNO > 0 AND SUP > 0 THEN 
    
    INSERT INTO EMPLOYEE VALUES
    (EMP_ID,BRANCH,DEPT,FIRST_NAME,LAST_NAME,SEX,DOB,SAL,EROLE,EID,CNO,SUP);
    
    DBMS_OUTPUT.PUT_LINE('EMPLOYEE DETAILS ENTERED');
...
/
                                      ...
                                      CREATE OR REPLACE PROCEDURE CUSTOMER_LOGIN(CUST IN NUMBER, FIRST_NAME IN VARCHAR, LAST_NAME IN VARCHAR, DOB IN DATE, WA IN VARCHAR, SEX IN VARCHAR, APT IN VARCHAR, STREET IN VARCHAR, CITY IN VARCHAR, CSTATE IN VARCHAR, ZIPCODE IN NUMBER, PHONE IN NUMBER)
AS
COUNT_C NUMBER := 0;
EXIST EXCEPTION;
INVALID EXCEPTION;
...
IF COUNT_C = 0 THEN
    ...
    IF (CUST > 0) AND LENGTH(APT) > 0 AND LENGTH(FIRST_NAME) > 0 AND LENGTH(LAST_NAME) > 0 AND LENGTH(WA) > 0 AND PHONE > 0 AND ZIPCODE > 0 AND LENGTH(CITY) > 0 AND LENGTH(STREET) > 0 AND LENGTH(CSTATE) > 0 THEN 
    
    INSERT INTO CUSTOMER VALUES
    (CUST,FIRST_NAME,LAST_NAME,DOB,WA,SEX,APT,STREET,CITY,CSTATE,ZIPCODE,PHONE);
    
    DBMS_OUTPUT.PUT_LINE('CUSTOMER DETAILS ENTERED');
